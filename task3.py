...
def names_find(names,names_ages):
  for item in names:
    try:
      print(f'{item} - {names_ages[item]}')
    except KeyError:
      print(f'Имени \'{item}\' в словаре не существует')
...
python_students=' , '.join(courses['python'])
javas_students=' , '.join(courses['javascript'])
java_students=' , '.join(courses['java'])
print(f'Python students : {python_students}')
print(f'Javascript students : {javas_students}')
